...
export const companyPeople = crmSchema.table(
  'company_people',
  {
    ...
    profileCaptureStatus: profileCaptureStatusEnum('profile_capture_status')
      .default('captured')
      .notNull(),
    profileNormalizationStatus: text('profile_normalization_status')
      .default('not_queued')
      .notNull(),
...
export const companyResearchJobs = crmSchema.table(
  ...
  (table) => [
    index('idx_company_research_jobs_company').on(table.companyId),
    index('idx_company_research_jobs_status').on(table.status),
  ]
);
...
export const talentosCompanySyncRuns = crmSchema.table(
  ...
  (table) => [
    index('idx_talentos_company_sync_runs_status').on(table.status),
    index('idx_talentos_company_sync_runs_created').on(table.createdAt),
  ]
);
