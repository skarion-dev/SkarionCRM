...
export default function CompanyPeoplePage({ category }: { category: CompanyPersonCategory }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[1250px] text-left text-sm"><thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Person</th><th className="px-4 py-3">LinkedIn</th><th className="px-4 py-3">Current company</th><th className="px-4 py-3">Title</th><th className="px-4 py-3">Location</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">Completeness</th><th className="px-4 py-3">Last captured</th></tr></thead><tbody className="divide-y divide-slate-100">
      {people.map((person) => <tr key={person.id} className="cursor-pointer hover:bg-slate-50" onClick={() => navigate(`/company-people/${person.id}`)}><td className="px-4 py-3"><Link to={`/company-people/${person.id}`} onClick={(e) => e.stopPropagation()} className="font-medium text-indigo-700 hover:underline">{person.display_name}</Link>{person.email && <div className="mt-1 text-xs text-slate-500">{person.email}</div>}<div className="mt-1 flex gap-1 text-[11px] text-slate-400">{person.categories.map((item) => <span key={item} className="rounded bg-indigo-50 px-1.5 py-0.5 text-indigo-600">{item.replace('_', ' ')}</span>)}</div></td><td className="px-4 py-3">{person.linkedin_url ? <a className="inline-flex items-center gap-1 text-indigo-600 hover:underline" href={person.linkedin_url} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}><ExternalLink size={14} />Open profile</a> : <span className="text-slate-400">Unlinked</span>}</td><td className="px-4 py-3 text-slate-700"><span className="inline-flex items-center gap-1"><Building2 size={14} />{person.current_company_name ?? 'Unlinked'}</span></td><td className="px-4 py-3 text-slate-600">{person.current_title ?? person.headline ?? '—'}</td><td className="px-4 py-3 text-slate-600">{person.location ?? '—'}</td><td className="px-4 py-3"><select className="rounded border border-slate-200 bg-white px-2 py-1 text-xs" value={category} onClick={(e) => e.stopPropagation()} onChange={(e) => updatePerson.mutate({ id: person.id, category: e.target.value as CompanyPersonCategory })}><option value="recruiter">Recruiter</option><option value="hiring_manager">Hiring Manager</option><option value="company_leadership">Leadership</option></select></td><td className="px-4 py-3"><div className="flex items-center gap-2"><div className="h-1.5 w-20 rounded bg-slate-200"><div className="h-1.5 rounded bg-indigo-500" style={{ width: `${Math.min(100, person.data_completeness ?? 0)}%` }} /></div><span className="text-xs text-slate-500">{person.data_completeness ?? 0}%</span></div></td><td className="px-4 py-3 text-slate-500">{person.last_captured_at ? new Date(person.last_captured_at).toLocaleString() : '—'}</td></tr>)}
