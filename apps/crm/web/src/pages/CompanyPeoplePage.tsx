...
const PAGE_CONFIG: Record<CompanyPersonCategory, { title: string; description: string }> = {
  recruiter: {
    title: 'Recruiters',
    description:
      'Recruiting professionals with lead-grade profile evidence, ownership, and capture history.',
  },
  hiring_manager: {
    title: 'Hiring Managers',
    description:
      'Engineering and hiring managers linked to companies and their complete profile context.',
  },
  company_leadership: {
    title: 'Company Leadership',
    description:
      'Leadership profiles for company intelligence, future research agents, and outreach.',
  },
};
              ...
              {people.map((person) => (
                ...
                >
                  <td className="px-4 py-3">
                    ...
                    {person.email && (
                      <div className="mt-1 text-xs text-slate-500">{person.email}</div>
                    )}
                    <div className="mt-1 flex gap-1 text-[11px] text-slate-400">
                      {person.categories.map((item) => (
                        <span
                          key={item}
                          className="rounded bg-indigo-50 px-1.5 py-0.5 text-indigo-600"
                        >
                          {item.replace('_', ' ')}
                        </span>
                      ))}
                  ...
                  <td className="px-4 py-3 text-slate-700">
                    <span className="inline-flex items-center gap-1">
                      <Building2 size={14} />
                      {person.current_company_name ?? 'Unlinked'}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <select
                      ...
                      onChange={(e) =>
                        updatePerson.mutate({
                          id: person.id,
                          category: e.target.value as CompanyPersonCategory,
                        })
                      }
                    >
                      <option value="recruiter">Recruiter</option>
                      <option value="hiring_manager">Hiring Manager</option>
                      <option value="company_leadership">Leadership</option>
                    </select>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 w-20 rounded bg-slate-200">
                        <div
                          ...
                          style={{ width: `${Math.min(100, person.data_completeness ?? 0)}%` }}
                        />
                      </div>
                      <span className="text-xs text-slate-500">
                        {person.data_completeness ?? 0}%
                      </span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-slate-500">
                    {person.last_captured_at
                      ? new Date(person.last_captured_at).toLocaleString()
                      : '—'}
                  </td>
                </tr>
              ))}
              {!isLoading && people.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-4 py-12 text-center text-slate-400">
                    <BriefcaseBusiness className="mx-auto mb-2" size={24} />
                    No {config.title.toLowerCase()} captured yet.
                  </td>
                </tr>
              )}
              {isLoading && (
