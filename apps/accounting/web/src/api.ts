        ...
        try {
          ...
          if (response.ok) {
            ...
            return {
              ...
              role: (data.apps?.books ?? '') as BooksRole,
...
export async function booksFetch<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  console.log(`[API] booksFetch: fetching ${url}...`);
  let response = await fetch(url, {
    ...init,
    headers,
  });
  console.log(`[API] booksFetch: ${path} response status:`, response.status);

  if (response.status === 401) {
    console.log('[API] booksFetch: token unauthorized (401), refreshing...');
    const refreshed = await refreshAccessToken();
    if (!refreshed) {
      console.warn('[API] booksFetch: refresh failed on 401, redirecting...');
      redirectToLogin();
      throw new ApiError('Session expired.', 401);
    }
    console.log(`[API] booksFetch: retrying ${path} with new token...`);
    ...
    console.log(`[API] booksFetch: retried ${path} response status:`, response.status);
  }

  if (!response.ok) {
    const body = await response.json().catch(() => ({ error: response.statusText }));
    console.error(`[API] booksFetch: request to ${path} failed:`, body.error);
