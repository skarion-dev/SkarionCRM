...
> **Superseded implementation note (dashboard audit):** Later migrations added a real
> `crm.activities.lead_id` foreign key and LinkedIn message imports now create lead-scoped activity
> rows. The recommendations below to remove `ActivityTimeline`/`ActivityForm` are therefore stale
> and must not be followed. The dashboard modernization itself has been implemented and subsequently
> hardened with backend-enforced team/personal scopes plus live automation/import reconciliation.
