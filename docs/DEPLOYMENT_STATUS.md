...
## Outreach Revamp (2026-07)

| Feature                                                        | Status                     |
| -------------------------------------------------------------- | -------------------------- |
| Per-platform outreach tracking (`lead_channels` table)         | ✅ Schema + API            |
| Import batch tagging (`import_batches` table, "Set 1"/"Set 2") | ✅ Schema + API + UI       |
| Lead attachments (R2-backed)                                   | ✅ Schema + API + UI       |
| Human-readable lead IDs (`leadNumber` L-00042)                 | ✅ Schema + backfill       |
| Full DB dedup (email → LinkedIn → name+company)                | ✅ Import preview + commit |
| Partner/manager assignment (ownerId visible/editable)          | ✅ UI                      |
| Outreach follow-up task auto-triggering                        | ✅ Workflow-runner + cron  |
| Workflow rules CRUD (Settings → Workflows)                     | ✅ UI                      |

### New CRM API endpoints

| Method | Path                                  | Purpose                                   |
| ------ | ------------------------------------- | ----------------------------------------- |
| POST   | `/api/leads/:id/outreach-actions`     | Log outreach attempt or set channel stage |
| GET    | `/api/leads/:id/channels`             | Per-platform channel rows                 |
| GET    | `/api/leads/:id/attachments`          | List attachments                          |
| POST   | `/api/leads/:id/attachments`          | Upload (R2, 25MB cap)                     |
| GET    | `/api/leads/attachments/:id/download` | Download from R2                          |
| DELETE | `/api/leads/attachments/:id`          | Delete attachment                         |
| GET    | `/api/import-batches`                 | List import sets for filter               |
...
- Tables: `crm.lead_channels`, `crm.import_batches`, `crm.lead_attachments`
- Enums: `crm.outreach_channel`, `crm.lead_channel_stage`
- Extended enums: `crm.activity_type` (+5 outreach types), `crm.lead_source` (+4 social), `crm.workflow_trigger` (+outreach_stale)
- Columns: `leads.lead_number`, `leads.batch_id`, `tasks.type`, `tasks.lead_id`
- Migration: `apps/crm/drizzle/0004_tricky_korath.sql` (with backfill + default rules)

### R2 bucket

- Name: `skarion-attachments`
- Binding: `ATTACHMENTS_BUCKET` on `skarion-crm-platform` Worker
- Create in Cloudflare dashboard before first deploy if not auto-created by wrangler.toml binding

## Employee Portal (HR) App

| Service       | Type              | URL                                                   |
| ------------- | ----------------- | ----------------------------------------------------- |
| HR Frontend   | Cloudflare Pages  | `https://skarion-hr.pages.dev` (create project first) |
| HR Worker/API | Cloudflare Worker | `https://skarion-hr-platform.alsaki1999.workers.dev`  |

- Deploy workflow: `.github/workflows/deploy-hr.yml`
- Books deploy workflow: `.github/workflows/deploy-books.yml`
